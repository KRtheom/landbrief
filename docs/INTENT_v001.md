...
Landbrief는 판단/해석/점수화 없이, 공공기관 부동산 데이터를 동일 기준·동일 기간으로 정렬해 사용자가 스스로 판단할 수 있게 만드는 지역/단지 비교 리포트 소프트웨어다.
...
+ ### 분기 데이터 yyyymm 규격 (수집 시 참고)
+ - 분기 지표: HF_KHAI, HF_PIR, HF_LIR
+ - HOUSTAT: 분기 시작월 저장 (202401=1Q, 202404=2Q, 202407=3Q, 202410=4Q)
+ - R-ONE: 원본 그대로 저장 (202501=1Q, 202502=2Q, 202503=3Q, 202504=4Q)
+ - DB에는 소스 원본 그대로, 서비스 레이어에서 통일 변환
...
| 9 | HF_PIR | 한국주택금융공사 HOUSTAT | 한국주택금융공사_지역별 PIR 및 LIR | ✅ 부담능력 | 분기 | HOUSTAT_API_KEY |
...
| 12 | SUPPLY_INCOMING | 공공데이터포털 | 한국부동산원_주택공급정보_입주예정물량정보 | ✅ 공급(스냅샷) | 반기 | DATA_GO_KR_API_KEY, odcloud 경유 |
...
섹션 5 [매크로/부담능력] 기준금리 시계열 (GLOBAL, 3지역 공통) PIR 시계열 (SIDO 레벨, 분기 — 비갱신 월은 직전값 캐리)
...
너는 Landbrief를 개발하는 코딩 파트너다. 우리는 판단/점수/예측을 제공하지 않고, 공공기관 데이터를 36개월 기준으로 정렬해 A/B/C 3지역 비교 리포트를 만든다. 웹 리포트가 기본이고, 유료 사용자만 PDF 다운로드를 제공한다. PDF는 요청 시 즉시 생성하고 동일 조건은 캐시 재사용한다. 코드/문서/출력에서 해석 문장(상승/침체/추천/위험 등)을 금지하고, 기관명_정식지표명 표기를 유지해라. docs/INTENT.md와 docs/DECISIONS.md를 반드시 참조해라.
