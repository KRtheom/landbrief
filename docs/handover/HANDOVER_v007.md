...
| 데이터 | 상태 | 비고 |
|---|---|---|
| 실거래 12종 | 완료 (각 19,832건 OK) | PRESALE 포함 (322,997건, apt_seq 없음) |
...
### 최우선
1. ENGINE.md 설계 (캘린더 테이블, 지표별 월/연 집계 규칙, 결측 처리, 입출력 구조)
2. DDL v0.3 확정 (calendar_month 추가)

### 중순위
3. 로컬 PostgreSQL DDL v0.3 적용 + 샘플 데이터 적재 테스트
4. UNSOLD 미분양 수집
5. 청약홈 분양정보 수집 + fact_supply_raw 설계
